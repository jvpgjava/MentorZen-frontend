...
import { EssayCreateRequest, EssayStatus } from '@/types';
import { showToast } from '@/utils/toast';
import Loading from '@/components/Loading';

const EditEssay: React.FC = () => {
    ...
    const loadEssay = async () => {
        if (!id) {
            showToast.error('ID da redação não fornecido', 'Erro');
            navigate('/essays');
            return;
        }
        ...
        try {
            ...
            const essay = await EssayService.getEssay(Number(id));
            ...
            setFormData({
                ...
                essayType: essay.essayType || 'ARGUMENTATIVE'
            });

            const words = (essay.content || '').trim().split(/\s+/).filter(word => word.length > 0);
            setWordCount(words.length);

            const themeInList = themes.find(t => t.value === essay.theme);
            if (themeInList) {
                setSelectedTheme(essay.theme || '');
            } else if (essay.theme) {
                setSelectedTheme('custom');
                setCustomTheme(essay.theme);
            }
        } catch (error: any) {
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            ...
            const essay = await EssayService.updateEssay(Number(id), essayData);
            updateEssay(essay);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (essayStatus === EssayStatus.DRAFT) {
                setIsLoading(true);
                try {
                    await EssayService.submitForAnalysis(Number(id));
                    showToast.success('Redação enviada para análise. Aguarde enquanto nossa IA analisa sua redação...', 'Redação Enviada');
                    navigate(`/essays/${id}/feedback`);
                } catch (submitError: any) {
                    showToast.error('Erro ao enviar para análise: ' + (submitError.message || 'Erro desconhecido'), 'Erro ao Enviar');
                    setIsLoading(false);
                }
            } else {
                showToast.success('Redação atualizada com sucesso', 'Redação Atualizada');
                navigate('/essays');
            }
        } catch (error: any) {
    ...
    const canEdit = essayStatus === EssayStatus.DRAFT || essayStatus === null;
    ...
    return (
        <div className="space-y-4">
            ...
            {!canEdit && (
                <Message
                    severity="warn"
                    text="Esta redação já foi submetida para análise e não pode ser editada"
                    className="w-full"
                />
            )}
            ...
            <Card className="shadow-lg border-0">
                <div className="p-6 space-y-6">
                    ...
                    {wordCount < 10 && formData.content && canEdit && (
                        <Message
                            severity="warn"
